...
s1 <- system2("git", c("--no-pager", "diff", "./R/01_vame_slot_nms.R"), stdout = TRUE)
local({
  lines <- readLines("./R/01_vame_slot_nms.R")
  ...
  writeLines(lines, "./R/01_vame_slot_nms.R")
})
Sys.sleep(1.0)
s2 <- system2("git", c("--no-pager", "diff", "./R/01_vame_slot_nms.R"), stdout = TRUE)
if (!identical(s1, s2)) {
  message("Create git commit on ./R/01_vame_slot_nms.R changes? [y/n]")
  if (readline(": ") == "y") {
    git2r::add(path = "./R/01_vame_slot_nms.R")
